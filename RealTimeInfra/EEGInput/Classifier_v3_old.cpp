...
int ClassifierV3(double data[N_DATA_POINTS*(N_CHANNELS+1)]) 
{
	...
	for (j=0;j<(N_DATA_POINTS / C_BIN_SIZE-1);j++)
	{
		...
		{
			channelDiff[j][i] = abs(diffData[j][i] - diffData[j][N_CHANNELS-i-1]);
		}
	}
	...
	for (i=0;i<N_CHANNELS/2;i++)
	{
		...
		if (channelDiff[(N_DATA_POINTS / C_BIN_SIZE-1)][i]/s>C_STD_LIM)
			passedThresh++;
...
{
	double m1=0., m2=0.;
	for (int i=0; i<n; i++) m1 += vals[i][ii];
	m1 /= n;
...
/*int main() {
    ...
    if (myfile.is_open()) {
		...
		cout << "Classifier returned " << Classifier(data) << endl;
        myfile.close();
	}
    else {
