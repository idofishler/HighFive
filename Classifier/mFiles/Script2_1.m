%load('C:\Users\galpert\Dropbox\HighFiveProject\Classifier\training sets\BrainHackDay1_Trial2-dataSet.mat')

%Run clasifier over time 
%Prior to running the script load eegData
classifierLength = 400;
isZoom = false;
isPlotAlternative=true;
...
for trial=1:nt
    ...
    for i=1:length(classifierT)
        ...
        if isPlotAlternative
            [classifierAnswer_Alt(i),classifierValue_Alt(i)] = ClassifyV3(d);
        end
        
        if classifierAnswer_Alt(i)==1
            disp('here');
        end
    end
    ...
    if isPlotAlternative
        plot (classifierT*1000,classifierAnswer_Alt*max(abs(classifierValue_Alt)),'g');
    end
    hold off
